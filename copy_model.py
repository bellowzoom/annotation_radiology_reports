...
class Seq2SeqWithCopyModel(nn.Module):
    ...
    def __init__(self, opt, emb_matrix=None):
        ...
        self.use_indi = opt.get('indication', True)
        ...
        dec_input_dim += self.hidden_dim if self.use_indi else 0
        ...
        if self.use_indi:
            self.indi_encoder = nn.LSTM(self.emb_dim, self.enc_hidden_dim, 1, \
                    bidirectional=True, batch_first=True, dropout=0) # when nlayer=1, rnn dropout does not apply
            self.indi_drop = nn.Dropout(self.dropout)
            self.indi_attn_layer = BasicAttention(self.hidden_dim)
    ...
    def decode(self, dec_inputs, dec_hidden, ctx, ctx_tokens, ctx_mask=None, indi_h=None, inference=False):
        ...
        if self.use_indi:
            dec_inputs_with_indi = torch.cat([dec_inputs, indi_h.unsqueeze(1).expand(-1, dec_inputs.size(1), -1)], dim=2)
        else:
            dec_inputs_with_indi = dec_inputs # no indi
        h, h_tilde, h_c, attn, cov, dec_hidden = self.decoder(dec_inputs_with_indi, dec_hidden, ctx, ctx_mask)
    ...
    def forward(self, src, tgt_in, indi):
        """
        ...
        if self.use_indi:
            ...
            indi_inputs = self.emb_drop(self.doc2vec(indi))
            indi_mask = indi.data.eq(constant.PAD_ID)
            indi_lens = indi_mask.eq(0).long().sum(1)
            indi_out, indi_hidden = self.encode(indi_inputs, indi_lens, encoder=self.indi_encoder, sort=True)
            indi_h = indi_hidden[0]
            _, indi_h, _ = self.indi_attn_layer(enc_hidden[0], indi_out, mask=indi_mask)
            indi_h = self.indi_drop(indi_h)

        else:
            indi_h = None

        out_log_probs, _, attn, cov = self.decode(dec_inputs, dec_hidden, h_in, src, ctx_mask=src_mask, indi_h=indi_h)
    ...
    def predict(self, src, indi, beam_size=5):
        ...
        if self.use_indi:
            indi_inputs = self.emb_drop(self.embedding(indi))
            indi_mask = indi.data.eq(constant.PAD_ID)
            indi_lens = indi_mask.eq(0).long().sum(1)
            indi_out, indi_hidden = self.encode(indi_inputs, indi_lens, encoder=self.indi_encoder, sort=True)
            indi_h = indi_hidden[0]
        ...
        with torch.no_grad():
            ...
            if self.use_indi:
                indi_h = indi_h.data.repeat(beam_size, 1)
        dec_hidden = (hn, cn)
        ...
        for i in range(self.max_dec_len):
            ...
            log_probs, dec_hidden, copy_indices = self.decode(dec_inputs, dec_hidden, h_in, src, ctx_mask=src_mask, indi_h=indi_h, inference=True)
